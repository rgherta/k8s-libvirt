...
# System timezone
timezone Europe/Rome --utc --ntpservers=it.pool.ntp.org,europe.pool.ntp.org
...
firewall-cmd --permanent --add-service=dhcp --add-service=dhcpv6 --add-service=dhcpv6-client --add-service=ssh --add-service=bgp --add-service=kubelet --add-service=kube-control-plane-secure
firewall-cmd --reload

# add admin user
useradd maintainer --create-home --groups wheel --shell /bin/bash --comment "maintainer user"
#mkdir /home/maintainer/.ssh/ && echo "${maintainer_public_key}" > /home/maintainer/.ssh/authorized_keys
